              ...
              <!-- <button data-toggle="modal" data-target="#modal-tambah" type="button" class="btn btn-md btn-primary btn-absolute">Tambah</button> -->
                <table id="tabledata" class="table  table-striped">
                  ...
                  <tr>
                    ...
                    <div class="btn-group">
                        @if($key->foto)
                        <span data-toggle="tooltip" data-placement="left" title="Lihat Foto">
                            <button onclick="modalImage('{{ucfirst($key->judul)}}','{{asset($key->foto)}}')" class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                        </span>
                        @else
                        <span data-toggle="tooltip" data-placement="left" title="Belum Ada Foto">
                            <button class="btn btn-sm btn-outline-success disabled"><i class="fas fa-eye"></i></button>
                        </span>
                        @endif

                        @if($key->foto_m)
                        <span data-toggle="tooltip" data-placement="left" title="Lihat Foto Mobile">
                            <button onclick="modalImage('{{ucfirst($key->judul)}}','{{asset($key->foto_m)}}')" class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                        </span>
                        @else
                        <span data-toggle="tooltip" data-placement="left" title="Belum Ada Foto">
                            <button class="btn btn-sm btn-outline-success disabled"><i class="fas fa-eye"></i></button>
            ...
            <div class="modal-body">
              <!-- <div class="card-body"> -->
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="judul_{{$key->id}}" name="judul[]" placeholder="Judul" value="{{$key->judul}}" readonly="readonly">
                </div>
                <div class="form-group">
                  <label>Foto <span class="input-keterangan">(jpg/jpeg/png)</span></label>
                  <div class="input-group">
                    <div class="custom-file">
                      <input type="file" class="custom-file-input input-foto" idlabel="label-photo-{{$key->id}}" name="photo[]" id="photo-{{$key->id}}">
                      <label id="label-photo-{{$key->id}}" class="custom-file-label">Pilih File</label>
                    </div>
                  </div>
                </div>

                <div class="form-group">
                  <label>Foto Mobile <span class="input-keterangan">(jpg/jpeg/png)</span></label>
                  <div class="input-group">
                    <div class="custom-file">
                      <input type="file" class="custom-file-input input-foto" idlabel="label-photo-m-{{$key->id}}" name="photo-m[]" id="photo-m-{{$key->id}}">
                      <label id="label-photo-m-{{$key->id}}" class="custom-file-label">Pilih File</label>
                    </div>
                  </div>
                </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <form method="post" id="formHapus_{{$key->id}}" class="form-horizontal">
            @csrf
            <div class="modal-body">
                <h6> Apakah anda ingin menghapus data slider {{$key->judul}}?</h6>
            </div>
            <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
      ...
      <div class="modal-content">
        ...
        <form method="post" id="_formData" class="form-horizontal">
          ...
          <div class="modal-body">
              ...
              <div class="form-group">
                <label>Foto<span class="bintang">*</span> <span class="input-keterangan">(jpg/jpeg/png)</span></label>
                <div class="input-group">
                  <div class="custom-file">
                    <input type="file" class="input-foto" id="photo_add" name="photo_add" idlabel="label-photo_add">
                    <label id="label-photo_add" class="custom-file-label" style="border-radius: .25rem;" for="photo_add">Pilih file</label>
                  </div>
                </div> 
              </div>
              <div class="form-group">
                <label>Foto Mobile<span class="bintang">*</span> <span class="input-keterangan">(jpg/jpeg/png)</span></label>
                <div class="input-group">
                  <div class="custom-file">
                    <input type="file" class="input-foto" id="photo_m_add" name="photo_m_add" idlabel="label-photo_m_add">
                    <label id="label-photo_m_add" class="custom-file-label" style="border-radius: .25rem;" for="photo_m_add">Pilih file</label>
                  </div>
                </div> 
              </div>
...
<script>
  $(document).ready(function(){
    ...
    $(document).on('click', '.btn-hapus', function (e) {
        ...
        var url = "{{ url('admin/hapusslider') }}/"+idform;
        $.ajaxSetup({
            headers: {
    ...
    $(document).on('click', '.btn-submit-data', function (e) {
        ...
        $('#formData_'+idform).validate({
          ...
          submitHandler: function () {
            ...
            var url = "{{ url('admin/updateslider') }}/"+idform;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
    ...
    $('#_formData').validate({
          ...
          messages: {
            ...
            photo_m_add: {
              required: "Foto mobile tidak boleh kosong"
            }
          },
          errorElement: 'span',
          errorPlacement: function (error, element) {
